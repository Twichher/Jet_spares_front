...
//tsc -b && vite build
    try {
      ...
      const response = await fetch(`http://${myurl}:4173/api/spares/list/?price_by=${Number(price_by_value)}&price_up=${Number(price_up_value)}`, {signal});
...
export const getSpareByID = async (
  ...
  Promise<MySpares> => {
    ...
    try {
      ...
      const response = await fetch(`http://${myurl}:4173/api/spares/${id_spare}/info/`, {signal})
